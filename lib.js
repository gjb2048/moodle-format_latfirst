...
function latfirst_init(wwwroot, moodleid, courseid, cookielifetime)
{
    // Init.
    thewwwroot = wwwroot;
...
function toggleexactweek(target,image,toggleNum,reloading)  // Toggle the target tr and change the image which is the a tag within the td of the tr above target
{
    if(document.getElementById)
    {
        ...
        if (target.style.display == displaySetting)
        {
            ...
            if (ie7OrLess == true)
            {
                target.className += " latest_first";  //add the class name
                //alert('Added class name');
            }
            image.style.backgroundImage = "url(" + thewwwroot + "/course/format/latfirst/arrow_down.png)";
            // Save the toggle!
        ...
        else
        {
            ...
            if (ie7OrLess == true)
            {
                target.className = target.className.replace(/\b latest_first\b/,'') //remove the class name
                //alert('Removed class name');
            }
            image.style.backgroundImage = "url(" + thewwwroot + "/course/format/latfirst/arrow_up.png)";
            // Save the toggle!
...
function saveweekcollcookie(value)
{
    ...
    if (cookieExpires == null)
    {
        ...
        YUI().use('cookie', function(Y){ 
           Y.Cookie.setSub("mdl_cf_latfirst",thecookiesubid,value); 
    ...
    else
    {
        ...
        YUI().use('cookie', function(Y){ 
            ...
            Y.Cookie.setSub("mdl_cf_latfirst",thecookiesubid,value, { expires: newDate }); 
           //alert("Bongo After " + thecookiesubid + " " + value);
        });
        // This is not a Moodle table but in fact the cookies name.
    }
...
function reloadToggles()
{
    YUI().use('cookie', function(daYUI){ 
        ...
        var storedval = daYUI.Cookie.getSub("mdl_cf_latfirst",thecookiesubid);
        if (storedval != null)
        {
