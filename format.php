...
<style type="text/css" media="screen">
    /* <![CDATA[ */
    @import url(<?php echo $CFG->wwwroot ?>/course/format/latfirst/latfirst.css);
    /* ]]> */
</style>
<!--[if lte IE 7]>
    <link rel="stylesheet" type="text/css" href="<?php echo $CFG->wwwroot ?>/course/format/latfirst/ie-7-hacks.css" media="screen" />
<![endif]-->

<?php
$PAGE->requires->js('/course/format/latfirst/lib_min.js');
$PAGE->requires->js_function_call('latfirst_init', array($CFG->wwwroot,
...
if (ajaxenabled() && $PAGE->user_is_editing()) {
    ...
    $PAGE->requires->js('/course/format/latfirst/lf_section_classes_min.js');
}

$week = optional_param('lf', -1, PARAM_INT);
...
echo '<colgroup><col class="left" /><col class="content" /><col class="right" style="' . get_string('latfirsttogglewidth', 'format_latfirst') . '" /></colgroup>';
...
$weektext = get_string('sectionname', 'format_latfirst');
$toggletext = get_string('latfirsttoggle', 'format_latfirst'); // The table row of the toggle.
// Toggle all.
echo '<tr id="toggle-all" class="section main">';
echo '<td class="left side toggle-all" colspan="2">';
echo '<h4><a class="on" href="#" onclick="all_opened(); return false;">' . get_string('latfirstopened', 'format_latfirst') . '</a><a class="off" href="#" onclick="all_closed(); return false;">' . get_string('latfirstclosed', 'format_latfirst') . '</a>' . get_string('latfirstall', 'format_latfirst') . '</h4>';
echo '</td>';
echo '<td class="right side">&nbsp;</td>';
echo '</tr>';
echo '<tr class="section separator"><td colspan="3" class="spacer"></td></tr>';
...
if (!$displaysection and $userisediting and has_capability('moodle/course:update', $context)) {
...
if (!empty($sectionmenu)) {
    $select = new single_select(new moodle_url('/course/view.php', array('id' => $course->id)), 'lf', $sectionmenu);
